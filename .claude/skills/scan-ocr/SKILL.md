---
name: scan-ocr
description: 書籍スキャンPDFや画像を lambda-ocr パイプラインで高精度日本語OCRする。「この本をOCR」「スキャンPDFをXLSX化」「日本語スキャンの精度を上げたい」等の要求時に使う。PaddleOCR(PP-OCRv5)+ プロジェクト固有の誤字辞書でTesseractより劇的に高精度。
---

# scan-ocr (lambda-ocr toolkit)
...
- `pdftoppm` (poppler)
- `~/Documents/dsgarageScript/lambda-ocr/` にツールキットが配置済み
...
```bash
cd ~/Documents/dsgarageScript/lambda-ocr
./setup.sh
```
