...
## 全体フロー (token-efficient 版 v0.3.0+)

```
  XLSX / JSON (OCR raw)
    ↓  fix_ocr() で機械的に直せる崩壊を吸収
  XLSX (fix_ocr 後)
    ↓  ocr_toolkit.suspicion.partition_by_threshold() ★ NEW
  ┌── suspicious (~30%): LLM 校正対象
  │     ↓  Claude Opus サブエージェント × 並列 2
  │   llm_corrections_<batch>.py (CORRECTIONS dict)
  │     ↓
  └── clean (~70%): そのまま採用 (LLM スキップ)

  両方を merge:
    ↓  ocr_toolkit.corrections.load_merged() + apply_corrections(callback=DB投入)
  本番 DB / 構造化出力
    ↓  ocr_toolkit.preview.write_comparison()
  比較 XLSX (目視レビュー)
```

**従来 (v0.2 まで) との違い**: 全行を LLM に送る代わりに、
`suspicion.partition_by_threshold` で「LLM に送る価値がある行」だけを抽出する。
PaddleOCR + 拡充版 fix_ocr の組み合わせで簡体字/促音/長音/末尾ノイズが事前処理され、
**LLM 入力行数を ~70% 削減** できる (実測値ベース)。

## 手順

### 0. 事前フィルタ (★ v0.3.0 新規)

OCR 直後に `fix_ocr` + `suspicion` で機械的に処理可能な行を切り分ける。
これだけで LLM 校正対象が ~70% 減るので、以降のコストを大幅に削減できる。

```python
from ocr_toolkit import fix_ocr, partition_by_threshold

# 1) fix_ocr で簡体字/促音/長音/末尾ノイズを機械的に直す
processed = [
    {**row,
     "lead": fix_ocr(row["lead"], extra_fixes=BOOK_SPECIFIC_FIXES),
     "leadText": fix_ocr(row["leadText"], extra_fixes=BOOK_SPECIFIC_FIXES)}
    for row in raw_rows
]

# 2) suspicion スコアで LLM 送付対象を抽出
suspicious, clean = partition_by_threshold(
    processed,
    text_fields=("lead", "leadText"),
    threshold=0.10,   # 0.05 (緩) 〜 0.20 (厳)、書籍ごとに調整
)

print(f"LLM 対象: {len(suspicious)} 行 ({100*len(suspicious)/len(processed):.0f}%)")
print(f"スキップ: {len(clean)} 行 (fix_ocr のみで採用)")

# 3) suspicious だけをダンプ → LLM へ
dump_for_llm(suspicious)
```

**閾値選択の指針**:
- 書籍 OCR 品質が高い (PaddleOCR server / VL) → 0.05〜0.10 で十分
- mobile + 縦書き混じり → 0.10〜0.20 (見落としリスクとトークンコストのバランス)
- 校正辞書を作り終えたら閾値を下げて再走 → 取りこぼし救済
...
- ❌ **suspicion フィルタを使わず全行を LLM に送る** → トークン浪費 (v0.3 以降は手順 0 を必ず通す)
- ❌ fix_ocr に蓄積エントリを追加せず、毎書籍 LLM で直す → 知見が他書籍に再利用されない

## トークン削減の優先順位 (v0.3.0+)

効果が大きい順:
1. **suspicion 事前フィルタ** (-70% LLM 入力行)
2. **書籍ごとの fix_ocr 拡充** (BOOK_SPECIFIC_FIXES → 共通辞書 OCR_FIXES へ昇格)
3. **プロンプトキャッシュ** (Claude Code の subagent はデフォルトで効く)
4. **バッチサイズ最大化** (suspicious 行数が少なければ 1 バッチに収める)
